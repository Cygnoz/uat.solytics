...
class ResponseHandler:
    def __init__(self):
        ...
        self.chat_history = {}  # Dictionary to store chat history per bot
    ...
    def get_response(self, user_input, bot_name, document_search):
        ...
        # Initialize chat history for this bot if it doesn't exist
        if bot_name not in self.chat_history:
        ...
        try:
            ...
            # Create the input dictionary with required keys
            chain_input = {
                "input_documents": docs,
                "question": user_input,
                "chat_history": self.chat_history[bot_name]
            }
            
            # Use the chain's invoke method instead of calling directly
            response = self.chain.invoke(chain_input)
            
            # Extract the answer from the response
            answer = response.get('output_text', "I couldn't find a relevant answer.")
            
            # Update chat history
            self.chat_history[bot_name].append((user_input, answer))
